endangered_simulation_nr <- function(plot){
  ...
  for(i in c(25,50,100)){
    ...
    tdat_frug1 <- tdat_frug2b[sample(which(tdat_frug2b$Endangered==1),replace=F, round(i*0.01*length(which(tdat_frug2b$Endangered==1)))), ]
    
    tdat_frugb <- anti_join(tdat_frug2b, tdat_frug1)
    
    tdat_frugc <- tdat_frug2b[sample(which(tdat_frug2b$Endangered==0),replace=T, (nrow(tdat_frug2b)-nrow(tdat_frugb))), ]
...
endangered_simulation_draws_nr <- function(FUNCTION_NAME){
  ...
  j
  write.csv(Plot_simulation, file=paste0(x=save_path, "Endangered/no_redundancy/output", j,".csv"))
