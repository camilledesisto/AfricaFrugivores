...
agb_simulation <- function(plot){
  ...
  for(i in c(25,50,100)){
    ...
    tdat_frug1 <- tdat_frug2b[sample(which(tdat_frug2b$DispersedSolo==1),replace=F, round(i*0.01*length(which(tdat_frug2b$DispersedSolo==1)))), ]
    
    tdat_frugb <- anti_join(tdat_frug2b, tdat_frug1)
    
    tdat_frugc <- tdat_frug2b[sample(which(tdat_frug2b$DispersedSolo==0),replace=T, (nrow(tdat_frug2b)-nrow(tdat_frugb))), ]
  ...
  plot_simulations <- sapply(rep(plot, each=10), FUN= agb_simulation) #Run the simulation
  Plot_simulation <- as.data.frame(abind(plot_simulations))
  Plot_simulation$Plot <- as.vector(rep(plot, each=30))
  Plot_simulation$PercentRemoved <- rep(c(25,50,100), times=2600)
  colnames(Plot_simulation)[1] <- "AGB_Change"
  write.csv(Plot_simulation, file=paste0(x=save_path, "total/redundancy/output", ".csv"))
