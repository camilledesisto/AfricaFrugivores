...
carnivore_simulation_nr <- function(plot){
  ...
  for(i in c(25,50,100)){
    ...
    tdat_frug1 <- tdat_frug2b[sample(which(tdat_frug2b$carnivore==1),replace=F, round(i*0.01*length(which(tdat_frug2b$carnivore==1)))), ]
    
    tdat_frugb <- anti_join(tdat_frug2b, tdat_frug1)
    
    tdat_frugc <- tdat_frug2b[sample(which(tdat_frug2b$carnivore==0),replace=T, (nrow(tdat_frug2b)-nrow(tdat_frugb))), ]
  ...
  plot_simulations <- sapply(rep(plot, each=100), FUN= carnivore_simulation_nr) #Run the simulation
  ...
  write.csv(Plot_simulation, file=paste0(x=save_path, "carnivore/no_redundancy/output", ".csv"))
