...
total_simulation_nr <- function(plot){
  ...
  for(i in c(25,50,100)){
    ...
    tdat_frugc <- tdat_frug2b[sample(which(tdat_frug2b$DispersedBinary==0),replace=T, (nrow(tdat_frug2b)-nrow(tdat_frugb))), ]
  ...
  plot_simulations <- sapply(rep(plot, each=25), FUN= total_simulation_nr) #Run the simulation
  plot_simulations2 <- sapply(rep(plot, each=25), FUN= total_simulation_nr)
  plot_simulations3 <- sapply(rep(plot, each=25), FUN= total_simulation_nr)
  plot_simulations4 <- sapply(rep(plot, each=25), FUN= total_simulation_nr)
  Plot_simulation <- as.data.frame(c(abind(plot_simulations), abind(plot_simulations2), abind(plot_simulations3),abind(plot_simulations4)))
  ...
  write.csv(Plot_simulation, file=paste0(x=save_path, "total/no_redundancy/output", ".csv"))
