...
ele_simulation_nr <- function(plot){
  ...
  for(i in c(25,50,100)){
    ...
    tdat_frug1 <- tdat_frug2b[sample(which(tdat_frug2b$ele==1),replace=F, round(i*0.01*length(which(tdat_frug2b$ele==1)))), ]
    
    tdat_frugb <- anti_join(tdat_frug2b, tdat_frug1)
    
    tdat_frugc <- tdat_frug2b[sample(which(tdat_frug2b$ele==0),replace=T, (nrow(tdat_frug2b)-nrow(tdat_frugb))), ]
  ...
  plot_simulations <- sapply(rep(plot, each=25), FUN= ele_simulation_nr) #Run the simulation
  plot_simulations2 <- sapply(rep(plot, each=25), FUN= ele_simulation_nr) #Run the simulation
  plot_simulations3 <- sapply(rep(plot, each=25), FUN= ele_simulation_nr) #Run the simulation
  plot_simulations4 <- sapply(rep(plot, each=25), FUN= ele_simulation_nr) #Run the simulation
  Plot_simulation <- as.data.frame(c(abind(plot_simulations),abind(plot_simulations2),abind(plot_simulations4),abind(plot_simulations4)))
  ...
  write.csv(Plot_simulation, file=paste0(x=save_path, "ele/no_redundancy/output", ".csv"))
